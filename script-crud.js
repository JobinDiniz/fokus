    ...
    const btnDeletarSelecionado = document.querySelector('#btn-remover-concluidas')
    ...
    const localStorageTarefas = localStorage.getItem('tarefas') //get seria retirar o que ja tem no storage

    let tarefas = localStorageTarefas ? JSON.parse(localStorageTarefas) : [] //transformando string em objeto


    const taskIconSvg = `<svg class="app__section-task-icon-status" width="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    btnDeleteAll.addEventListener('click', () => removerTarefas(false))
    ...
    cancelBt.addEventListener('click', () => {
        formLabel.textContent = 'Cancelando tarefa'
        formTask.classList.toggle('hidden')
    })

    cancelBt.addEventListener('click', limparForm)



    btnDeletar.addEventListener('click', apagarSelecionado)

    btnDeletarSelecionado.addEventListener('click', removerTarefas )



    const updateLocalStorage = () => {
        localStorage.setItem('tarefas', JSON.stringify(tarefas)) // transformando objeto para string
    }
